...
def estimate_file_size(full_diapazon):
    ...
    avg_line_length = 200  # Увеличили из-за добавления метода относительных величин
    header_lines = 10
...
if save_to_file == 'yes':
    with open(filename, 'w', encoding='utf-8') as file:
        ...
        for i, a in numbers:
            ...
            A_relative = relative_values_method(osn_inform, osn_kontrol, a, precision)
            file.write(f"Метод относительных величин: A = {A_relative}\n")
            
            #Сравнение результатов
            if i == A_orth == A_opss == A_combined:
                file.write("Все точные методы дали корректный результат\n")
            else:
...
if user_input == 'yes':
    ...
    for i, a in numbers:
        ...
        if i == A_orth == A_opss == A_combined:
            print("Все точные методы дали корректный результат")
        else:
...
print(f'\nПиковое использование памяти: {peak / 1024:.2f} KB')
